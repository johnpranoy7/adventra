...
export default function Home() {
  ...
  const adventures = [
    { id: 1, name: "Scuba Diving", location: "Redang Island, Malaysia", image: "/scuba.jpg" },
    { id: 2, name: "Mountain Biking", location: "Rocky Mountains, USA", image: "/biking.jpg" },
    { id: 3, name: "Safari", location: "Serengeti, Tanzania", image: "/safari.jpg" },
  ];
      ...
      <section className="relative w-full h-full flex justify-center items-center overflow-hidden">
        <AnimatePresence mode="popLayout" initial={false}>
          <motion.div
            ...
            style={{ backgroundImage: `url(${adventures[currentIndex].image})` }}
            drag="x"
            dragConstraints={{ left: 0, right: 0 }}
            dragElastic={0.0}
            onDragEnd={(event, info) => {
              if (info.offset.x < -0.1) handleSwipe("right"); // Match
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center text-center text-white px-6">
              <h2 className="text-4xl font-bold">{adventures[currentIndex].name}</h2>
